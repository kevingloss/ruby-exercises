RSpec.describe 'select' do
  ...
  it 'words with three letters' do
    ...
    selected = words.select do |filter|
      filter.length == 3
    end
  ...
  it 'hashes' do
    ...
    hashes = elements.select do |filter|
      filter if filter.class == Hash
    end
